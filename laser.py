...
### M18 disable stepper motors and allow axes to move freely
...
class laser_gcode(inkex.Effect):
    ...
    def export_gcode(self,gcode):
        ...
        f.write(self.options.laser_off_command +" "+self.options.laser_power_cmd + "0" + "\n" + self.header + "G1 F" + self.options.travel_speed + "\n" + gcode + self.footer)
        f.close()

    def __init__(self):
        inkex.Effect.__init__(self)
        self.OptionParser.add_option("-d", "--directory",                       action="store", type="string",          dest="directory",                           default="",                             help="Output directory")
        self.OptionParser.add_option("-f", "--filename",                        action="store", type="string",          dest="file",                                default="output.nc",    	            help="File name")            
        ...
        self.OptionParser.add_option("",   "--horizontal-only",                 action="store", type="inkbool",         dest="horizontal_only",                     default=False,                          help="Laser horizontal lines only")
        ...
        self.OptionParser.add_option("",   "--laser-power-cmd",                 action="store", type="string",          dest="laser_power_cmd",                     default="S",                            help="S or might be L")
        self.OptionParser.add_option("",   "--laser-power",                     action="store", type="int",             dest="laser_power",                         default="256",                          help="256 for full laser power")
...
#############################################################################################################################
##### Turn off laser on all vertical movements. editor = Cale Kaupp 6/26/16
#############################################################################################################################		
        if self.options.horizontal_only :
            loopcount = len(curve)
            index = 0
            while index < loopcount:
                if curve[index][1] != 'line' or round(curve[index][0][1],3) != round(curve[index][4][1],3) or round(curve[index][0][0],3) == round(curve[index][4][0],3):
                    del curve[index]
                    loopcount -= 1
#make sure each individual laser run that laser moves from the left to right only
                elif curve[index][0][0]>curve[index][4][0]: 
                    end = round(curve[index][0][0],3)
                    curve[index][0][0]=round(curve[index][4][0],3)
                    curve[index][4][0]=end
                    curve[index][0][1]=round(curve[index][0][1],3)
                    curve[index][4][1]=round(curve[index][4][1],3)
                    index += 1
                else:
                    curve[index][0][0]=round(curve[index][0][0],3)
                    curve[index][0][1]=round(curve[index][0][1],3)
                    curve[index][4][0]=round(curve[index][4][0],3)
                    curve[index][4][1]=round(curve[index][4][1],3)
                    index += 1
# make sure laser moves from bottom to top only and far left to right
            curve.sort(key=lambda x: (x[0][1],x[0][0]))
            loopcount = len(curve)
            index = 0
            right = True
            vert = curve[index][0][1]
            array = []
            while index < loopcount:
                arrayindex = len(array)+index
                if arrayindex >= loopcount:
                    while array:
                        curve.insert(index,array.pop())
                        index += 1
                elif curve[index][0][1] != vert:
                    right = not right
                    vert = curve[index][0][1]
                    while array:
                        curve.insert(index,array.pop())
                        index += 1
                elif right == False:# and index < loopcount:
                    end = curve[index][0][0]
                    curve[index][0][0]=curve[index][4][0]
                    curve[index][4][0]=end
                    array.append(curve[index])
                    del curve[index]
                else:
                    index += 1
            ...
            index = 0
            while index < loopcount:
                curve.insert(index+1,[curve[index][4],'end',0,0])
                curve.insert(index,[curve[index][0],'move',0,0])
                index += 3
                loopcount += 2
...
################################################################################
    def effect(self) :
        ...
        self.tools = {
            ...
            "gcode before path": ("G4 P0 \n" + self.options.laser_command + " " + self.options.laser_power_cmd + str(int(self.options.laser_power)) + "\nG4 P" + self.options.power_delay),
			"gcode after path": ("G4 P0 \n" + self.options.laser_off_command + " "+self.options.laser_power_cmd+"0" + "\n" + "G1 F" + self.options.travel_speed),
        }
